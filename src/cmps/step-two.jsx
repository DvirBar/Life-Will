...
export default function StepTwo({ formikProps }) {
	debugger;
	const {
		moveNextStep
	...
	return (
		<>
			...
			<div role="group" className="status-group flex space-between input-btn">
				<label className={`${formikProps.values.status === "נשוי" ? 'active' : ''}`}>
					<Field type="radio" name="status" value="נשוי" />
					נשוי
				</label>
				<label className={`${formikProps.values.status === "פרוד" ? 'active' : ''}`}>
					<Field type="radio" name="status" value="פרוד" />
					פרוד
				</label>
				<label className={`${formikProps.values.status === "אלמן" ? 'active' : ''}`}>
					<Field type="radio" name="status" value="אלמן" />
					אלמן
				</label>
				<label className={`${formikProps.values.status === "רווק" ? 'active' : ''}`}>
					<Field type="radio" name="status" value="רווק" />
					רווק
				</label>
				<label className={`${formikProps.values.status === "ערירי" ? 'active' : ''}`}>
					<Field type="radio" name="status" value="ערירי" />
					ערירי
				</label>
				<label className={`${formikProps.values.status === "שותפות" ? 'active' : ''}`}>
					<Field type="radio" name="status" value="שותפות" />
					שותפות
				</label>
				<label className={`${formikProps.values.status === "גרוש" ? 'active' : ''}`}>
					<Field type="radio" name="status" value="גרוש" />
					גרוש
				</label>
			</div>
			{formikProps.values.status === 'נשוי' && <div>
				<div className='partner-details'>
					<h3>אני נשוי ל-</h3>
					<div role="group" className="status-group flex space-between input-btn">
						<label className={`${formikProps.values.partner_gender === "גבר" ? 'active' : ''}`}>
							<Field type="radio" name="partner_gender" value="גבר" />
							גבר
						</label>
						<label className={`${formikProps.values.partner_gender === "אישה" ? 'active' : ''}`}>
			...
			{formikProps.values.status === 'שותפות' && <div>
				<div className='partner-details'>
			...
			{formikProps.values.status === 'גרוש' && <div>
				<div role="group" className="status-group flex space-between input-btn">
					<h5>האם תרצה להקצות  לגרוש/גרושתך מהצוואה?</h5>
					<label className={`${formikProps.values.ex_partner_gain === "לא" ? 'active' : ''}`} style={{ width: '40px' }}>
						<Field type="radio" name="ex_partner_gain" value="לא" />
						לא
					</label>
					<label className={`${formikProps.values.ex_partner_gain === "כן" ? 'active' : ''}`} style={{ width: '40px' }}>
						<Field type="radio" name="ex_partner_gain" value="כן" />
						כן
					</label>
				</div>
				{formikProps.values.ex_partner_gain === 'כן' && <>
					<div className='partner-details'>
						<h3>פרטי זיהוי–</h3>
						<div role="group" className="status-group flex space-between input-btn">
							<label className={`${formikProps.values.partner_gender === "גבר" ? 'active' : ''}`}>
